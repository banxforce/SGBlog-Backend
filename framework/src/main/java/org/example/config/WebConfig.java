...
public class WebConfig implements WebMvcConfigurer {
    ...
    /**
     *虽然MVC和Security都使用了 CorsFilter，但它们是独立的实例，分别用于处理 Spring MVC 和 Spring Security 中的请求。
     * 在绝大多数情况下，只需要在其中一个地方配置 CORS，然后这个配置就会应用到整个应用程序中。
     * 如果同时在 Spring MVC 和 Spring Security 中都配置了 CORS，通常以 Spring Security 配置为准，因为它在过滤器链中的位置可能更靠前。
     */
